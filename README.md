...
### 2.2 硬件准备
- 开发板：二块以上 ESP32-DevKitC 开发板（测试零配网功能至少需要两块开发板）
...
## 4 文件结构
    ...
    ├── sdkconfig                               // 保存配置选项
    └── sdkconfig.old
> esp32-alink-embed 文件结构及其详解，参见 `esp32-alink-embed/README.md`

## 5 编译环境的搭建
如果您是在 ubuntu x64bit 的平台下开发只需运行脚本 `setup_toolchain.sh`，就可以完成开发环境的搭建。若是其他平台请参见：http://esp-idf.readthedocs.io/en/latest/get-started/linux-setup.html
...
# 安装软件
sudo apt-get install git wget make libncurses-dev flex bison gperf python python-serial # 安装常用软件
...
## 6 配置
ESP32 ALINK EMBED 的默认配置已经存储在 sdkconfig， 如果你想要详细了解esp32相关配置与编译详见 esp-idf/README.md
...
> 注：除非您有特殊需求，否则您在开发时只需修改 sample 下的代码，无需关心其内部实现细节
...
2. **配网**
   - 事件回调函数：
        ...
        - 事件回调函数的栈默认大小为 4k, 请避免在事件回调函数使用较大的栈空间，如需修改请使用 `make menuconfig`
...
4. **数据通信**
    ...
    - 数据长度：默认长度为1KB，数据长度最大支持1.5k，如需修改请使用 `make menuconfig`
    - 数据上报：主动上报是由设备端主动发起
